...
class ScheduleController{
    ...
    public intializeRoutes() {
      ...
      getSchedule = (request: express.Request, response: express.Response) => {
        ...
        const flights = generateFlights(aircraftCount,flightCount, startDate, endDate);
...
function generateFlights(aircraftCount: number, flightsCount: number, start: luxon.DateTime, end: luxon.DateTime){
    ...
    for(var i=0; i<aircraftCount; i++){
        ...
        for(var j=0; j<flightsCount; j++){
            ...
            while (thisFlight.arrival_airport.iata == thisFlight.departure_airport.iata) { thisFlight.arrival_airport = new Airport().random() }
            ...
            while (thisFlight.crewmembers[0].id == thisFlight.crewmembers[1].id){ thisFlight.crewmembers[1] = new Crewmember().random() }
